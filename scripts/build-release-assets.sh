...
lake build
test "$(.lake/build/bin/agent-workbench --version)" = "agent-workbench $version"
...
cp .lake/build/bin/agent-workbench "$stage/binary/agent-workbench"
...
tar -xzf "$out/agent-workbench-$tag-linux-x86_64.tar.gz" -C "$stage/verify"
