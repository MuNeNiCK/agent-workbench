...
fn require_doctor_schema(conn: &Connection) -> Result<()> {
...
fn work_unit_project(conn: &Connection, id: i64) -> Result<Option<i64>> {
    conn.query_row(
...
fn task_scope(conn: &Connection, id: i64) -> Result<Option<(i64, Option<i64>)>> {
    conn.query_row(
        r#"
...
fn snapshot_project(conn: &Connection, id: i64) -> Result<Option<i64>> {
    conn.query_row(
        r#"
        select r.project_id
...
fn unknown_validation_run_references(conn: &Connection) -> Result<Vec<(String, String)>> {
...
fn ensure_audit_schema(conn: &Connection) -> Result<()> {
    conn.execute_batch(
        r#"
...
fn next_backup_path(root: &Path) -> Result<PathBuf> {
