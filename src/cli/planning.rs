...
pub(crate) fn handle_decision(root: &Path, command: DecisionCommand) -> Result<()> {
    match command {
        ...
        DecisionCommand::Capability { command } => match command {
            DecisionCapabilityCommand::Issue(args) => {
                let outcome = issue_capability(
                    root,
                    CapabilityIssueRequest {
                        principal_handle: &args.principal,
                        owner_grant: &args.owner_grant,
                        assertion_handle: &args.issuer_assertion,
                ...
                println!("capability_handle: {}", outcome.capability_handle);
            }
        },
        DecisionCommand::Adjudicate(args) => {
            let outcome = present_decision(
                root,
                OwnerDecisionRequest {
                    command_kind: "decision adjudicate",
                    principal_handle: &args.principal,
                    capability_handle: &args.capability,
                    owner_ref: &args.owner,
                    target_ref: &args.target,
                    decision_family: &args.decision_family,
                    action: &args.action,
        ...
        DecisionCommand::Continuation { command } => match command {
            DecisionContinuationCommand::Show(args) => {
                let output = show_decision_continuation(root, &args.handle)?;
                println!("continuation: {}", output.continuation_handle);
                println!("status: {}", output.status);
                println!("command: {}", output.command_kind);
                println!("owner: {}", output.owner_ref);
                println!("target: {}", output.target_ref);
                println!("decision_family: {}", output.decision_family);
                println!("action: {}", output.action);
                println!("expected_current: {}", output.expected_current);
                println!("design_context: {}", output.design_context);
                println!("required_input: decision,reason,principal,capability");
            }
            DecisionContinuationCommand::Apply(args) => {
                let output = apply_decision_continuation(
                    root,
                    DecisionContinuationApplyRequest {
                        continuation_handle: &args.handle,
                        decision_value: &args.decision,
                        reason: &args.reason,
                        principal_handle: &args.principal,
                        capability_handle: &args.capability,
