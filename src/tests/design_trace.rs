...
fn reconciliation_inherits_completed_membership_from_closed_phase() {
    ...
    .unwrap();
    let inherited_usage = add_command_usage(
        temp.path(),
        NewCommandUsage {
            profile: None,
            command: Some("manual GATE-001 validation"),
    ...
    add_validation_run(
        ...
        NewValidationRun {
            ...
            command_usage_id: Some(inherited_usage.command_usage_id),
            repository_snapshot_id: None,
            result: "pass",
            command: None,
            classification: None,
    ...
    assert!(
        selected_mismatch
            .to_string()
            .contains("lacks qualifying evidence")
    );
    assert_eq!(
    ...
    for (mutate, restore) in lifecycle_cases {
