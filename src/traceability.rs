...
pub(crate) fn inherit_closed_phase_memberships_in(
        ...
        rows
    {
            ...
            .query_row(
                ...
                where candidate_v.design_package_id=?1
                  ...
                  and exists(select 1 from authority_events candidate_approval
                             where candidate_approval.id=candidate_v.approved_by_authority_event_id
                               and candidate_approval.project_id=?4
                               and candidate_approval.status='active')
                  and (select count(*) from design_requirements candidate_r
                       where candidate_r.design_version_id=candidate_v.id
                         and candidate_r.requirement_key=?3)=1
        ...
        let mut source_stmt = conn.prepare(
            ...
            from design_requirements source_r
            join design_versions source_v on source_v.id=source_r.design_version_id
            join task_derivations td on td.design_requirement_id=source_r.id
            join tasks t on t.id=td.task_id
            ...
            where source_v.design_package_id=?1
              and source_v.version_number < (select version_number from design_versions where id=?2)
              and source_v.status in ('approved','superseded')
              and source_v.approved_by_authority_event_id is not null
              and source_v.approved_at is not null
              and exists(select 1 from authority_events approval
                         where approval.id=source_v.approved_by_authority_event_id
                           and approval.project_id=?8 and approval.status='active')
              and source_v.id=?10
              and source_r.requirement_key=?3
              and (select count(*) from design_requirements same_key
                   where same_key.design_version_id=source_v.id
                     and same_key.requirement_key=?3)=1
              and source_r.revision=?4
              and source_r.requirement_hash=?5
              and source_r.required_surfaces is ?6
              and t.work_unit_id=?7 and t.status='closed'
              and td.status='closed'
              and source_ci.status='closed'
              and p.project_id=?8 and p.work_unit_id=?7 and p.status='closed'
              and p.authority_event_id is null
              and p.closed_at is not null and m.assigned_at<=p.closed_at
              and (select count(*) from task_derivations exact_td
                   where exact_td.design_requirement_id=source_r.id)=1
              and (select count(*) from checklist_items exact_ci
                   where exact_ci.id=td.checklist_item_id and exact_ci.task_id=t.id
                     and exact_ci.design_requirement_id=source_r.id and exact_ci.status='closed')=1
              and (select count(*) from work_phase_task_memberships exact_m
                   where exact_m.task_id=t.id)=1
              and (select count(*) from work_phase_events e
                   where e.phase_id=p.id and e.event_type='closed')=1
              and exists(select 1 from work_phase_events e
                         where e.phase_id=p.id and e.event_type='closed'
                           and e.created_at=p.closed_at)
              and exists(select 1 from implementation_evidence ie
                         where ie.task_id=t.id and ie.design_requirement_id=source_r.id
                           and ie.created_at<=p.closed_at)
              and (select count(*) from coverage_items source_c
                   where source_c.task_id=t.id and source_c.design_requirement_id=source_r.id
                     and source_c.status='covered' and source_c.created_at<=p.closed_at)=1
              and not exists(
                select 1 from validation_gate_template_requirements current_map
                join validation_gate_templates current_gt
                  on current_gt.id=current_map.validation_gate_template_id
                ...
                where current_map.design_requirement_id=?9
                  and not exists(
                    ...
                    join validation_gate_templates source_gt
                      on source_gt.id=source_map.validation_gate_template_id
                     and source_gt.design_version_id=source_r.design_version_id
                     and source_gt.status='active'
                    where source_map.design_requirement_id=source_r.id
                      and source_gt.gate_key=current_gt.gate_key
                      and source_gt.gate_hash=current_gt.gate_hash
                  )
              )
              and not exists(
                select 1 from validation_gate_template_requirements source_map
                ...
                where source_map.design_requirement_id=source_r.id
                  and not exists(
                    ...
                    join validation_gate_templates current_gt
                     ...
                     and current_gt.design_version_id=?2 and current_gt.status='active'
                    where current_map.design_requirement_id=?9
                      and current_gt.gate_key=source_gt.gate_key
                      and current_gt.gate_hash=source_gt.gate_hash
                  )
              )
              and not exists(
                select 1 from validation_gates source_gate
                join validation_gate_templates source_gt on source_gt.id=source_gate.template_id
                where source_gate.task_id=t.id
                  and source_gate.design_requirement_id=source_r.id
                  and source_gate.selected_before_edit=1
                  and not exists(
                    select 1 from validation_runs latest
                    where latest.id=(select max(candidate.id) from validation_runs candidate
                                     where candidate.validation_gate_id=source_gate.id
                                       and candidate.created_at<=p.closed_at)
                      and latest.result='pass' and latest.command_usage_id is not null
                      and exists(select 1 from command_usages usage
                                 where usage.id=latest.command_usage_id and usage.project_id=?8
                                   and usage.work_unit_id=?7 and usage.result='pass'
                                   and (source_gate.command is null or usage.command=source_gate.command))
                  )
              )
        ...
        drop(source_stmt);
        if sources.is_empty() {
            let incomplete_source: i64 = conn.query_row(
                r#"select count(*) from design_requirements source_r
                    join design_versions source_v on source_v.id=source_r.design_version_id
                    join task_derivations td on td.design_requirement_id=source_r.id
                    join tasks t on t.id=td.task_id
                    join checklist_items ci on ci.id=td.checklist_item_id
                    ...
                    join work_phases p on p.id=m.phase_id
                    where source_v.id=?1
                      and source_r.requirement_key=?2
                      and t.work_unit_id=?3 and p.work_unit_id=?3"#,
                params![selected_baseline, key, work_unit_id],
                |row| row.get(0),
            )?;
            if incomplete_source > 0 {
                bail!("closed-phase completion source lacks qualifying evidence for {key}");
            }
            continue;
        }
        if sources.len() != 1 {
        ...
        let canonical_memberships: i64 = conn.query_row(
            "select count(*) from work_phase_task_memberships where task_id=?1",
            params![canonical_task],
            |row| row.get(0),
        )?;
        if canonical_memberships > 0 {
            bail!("canonical task is already phase-assigned for {key}");
        }
        let (
            source_task,
            source_item,
        ...
        conn.execute(
            "delete from work_phase_task_memberships where id=?1",
            params![source_membership],
        )?;
        conn.execute(
            r#"insert into work_phase_task_memberships(project_id, phase_id, task_id, assigned_at)
               values (?1, ?2, ?3, current_timestamp)"#,
            params![project_id, phase_id, canonical_task],
        )?;
        conn.execute(
