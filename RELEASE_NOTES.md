# Agent Workbench v0.2.4

`v0.2.4` replaces the prior workflow with a Lean-assisted project workflow.
...
- Select formal, external-evidence, mixed, or no assurance per accepted design
  statement.
- Acquire the pinned official Lean distribution during `init` without a
...
- Install as one Agent Skill with a Linux x86_64 runtime.
