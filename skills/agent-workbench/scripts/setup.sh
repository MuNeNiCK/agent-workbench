...
destination="$project_root/.agent-workbench/bin"
runtime="$destination/agent-workbench"
runtime_release="$destination/skill/agent-workbench/release-version"
needs_install=true
if [ -x "$runtime" ] && [ -f "$runtime_release" ] &&
    [ "$(sed -n '1p' "$runtime_release")" = "$release_version" ]; then
  needs_install=false
fi
...
if [ "$needs_install" = true ]; then
  ...
  chmod +x "$runtime" "$destination/elan"
  test "$(sed -n '1p' "$runtime_release")" = "$release_version"
fi
